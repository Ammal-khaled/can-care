...
const inRange = (dateStr, start, end) => {
  ...
  // ---- Fictional demo seed data for portfolio safety
...
const ChiefDashboard = () => {
  ...
  const pendingTransfers = transfers.filter((t) => (t.status ?? "Pending") === "Pending");
  const pendingReviewCount = notifications.length + waitlist.length + pendingTransfers.length;
  ...
  const needsAttentionItems = [
    {
      status: "Pending",
      statusClass: "pending",
      meta: `${pendingReviewCount} open`,
      title: "Pending notifications or requests",
      description: pendingReviewCount
        ? "Review demo notifications, waitlist entries, or transfer requests awaiting admin attention."
        : "No pending demo requests are open right now.",
      actionLabel: "Open notifications",
      onAction: () => navigate("/notifications?status=pending"),
    },
    {
      status: "Cancelled",
      statusClass: "cancelled",
      meta: `${cancelledInRange.length} found`,
      title: "Cancelled appointments",
      description: cancelledInRange.length
        ? "Cancelled appointments in the selected range may need follow-up or rescheduling."
        : "No cancelled appointments in the selected range.",
      actionLabel: "View appointments",
      onAction: () => navigate("/appointments?status=cancelled"),
    },
    {
      status: "Missing info",
      statusClass: "missing-info",
      meta: `${missingAssignments.length} records`,
      title: "Patients missing assigned doctor or nurse",
      description: missingAssignments.length
        ? "Some fictional patient records do not yet have a complete care team assignment."
        : "All visible demo patients have assignment information.",
      actionLabel: "Review",
      onAction: () => navigate("/patients?filter=missing-assignment"),
    },
    {
      status: "Today",
      statusClass: "today",
      ...
      description: appointmentsToday.length
        ? "Today's appointment queue is ready for admin oversight."
        : "No appointments are scheduled for today in the demo data.",
      actionLabel: "View appointments",
      onAction: () => navigate("/appointments?filter=today"),
    },
    {
      status: "Review",
      statusClass: "review",
      meta: latestPost ? "1 recent" : "Demo-ready",
      title: "Recent community activity",
      description: latestPost
        ? `Latest support content: ${latestPost.title}.`
        : "Community posts can be created and reviewed from the admin workspace.",
      actionLabel: "Review",
    ...
    <div className="chief-page">
      ...
      <header className="chief-header">
        <div>
          ...
          <p>Admin oversight for fictional cancer care operations</p>
        </div>
  ...
  return (
    <div className="chief-overlay">
      <div className="chief-modal" style={{ maxWidth: 480 }}>
        ...
        <div className="chief-modal-body grid2 modal-form-grid">
