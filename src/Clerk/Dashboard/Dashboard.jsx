...
const ClerkDashboardModern = () => {
  ...
  const appointmentsToday = appointments.filter((a) => a.date === todayKey);
  const missingAssignments = patients.filter(
    (p) => !p.doctorId && !p.doctor && !p.assignedDoctorId && !p.nurseId && !p.nurse && !p.assignedNurseId
  );
  const nextAvailableSlots = doctors.flatMap((doc) =>
    next7Days.flatMap((date) =>
      availableSlots(doc.name, date).map((time) => ({
        doctor: doc.name,
        date,
        time,
      }))
  ...
  const latestRegistration = patients[0];
  const firstOpenSlot = nextAvailableSlots[0];
  ...
  const openFirstAvailableSlot = () => {
    ...
    const doctor = doctors.find((d) => d.name === firstOpenSlot.doctor);
    navigate(`/clerk/appointments?action=book&doctorId=${doctor?.id || ""}&date=${firstOpenSlot.date}`);
  };
  ...
  const needsAttentionItems = [
    {
      status: "Today",
      statusClass: "today",
      meta: `${appointmentsToday.length} visits`,
      title: "Today's appointments",
      description: appointmentsToday.length
        ? "Review today's registration desk appointment queue."
        : "No appointments are scheduled for today in the demo data.",
      actionLabel: "View appointments",
      onAction: () => navigate("/clerk/appointments?filter=today"),
    },
    {
      status: "Missing info",
      statusClass: "missing-info",
      meta: `${missingAssignments.length} records`,
      title: "Patients missing doctor/nurse assignment",
      description: missingAssignments.length
        ? "Some fictional patient records need a complete care team assignment."
        : "All visible demo patients have assignment information.",
      actionLabel: "Review",
      onAction: () => navigate("/clerk/patients?filter=missing-assignment"),
    },
    {
      status: "Review",
      statusClass: "review",
      meta: `${nextAvailableSlots.length} slots`,
      title: "Available appointment slots",
      description: firstOpenSlot
        ? `Next open slot: ${firstOpenSlot.doctor}, ${firstOpenSlot.date} at ${firstOpenSlot.time}.`
        : "No open appointment slots are available in the next 7 demo days.",
      actionLabel: "Book slot",
      onAction: openFirstAvailableSlot,
    },
    {
      status: "Pending",
      statusClass: "pending",
      meta: `${patients.length} intakes`,
      title: "Recent registration activity",
      description: latestRegistration
        ? `Latest fictional intake record: ${latestRegistration.name}.`
        : "No recent registration activity is available in demo data.",
      actionLabel: "Review",
      onAction: () => navigate(`/clerk/patients?search=${encodeURIComponent(latestRegistration?.name || "")}`),
    },
    {
      status: "Cancelled",
      statusClass: "cancelled",
      meta: `${cancelledVisits.length} found`,
      title: "Cancelled appointments that may need rescheduling",
      description: cancelledVisits.length
        ? "Cancelled appointments in the selected range may need clerk follow-up."
        : "No cancellations need rescheduling in the selected range.",
      actionLabel: "View appointments",
      onAction: () => navigate("/clerk/appointments?status=cancelled"),
    },
  ];
    ...
    <div className="clerk-dashboard">
      ...
      <div className="dashboard-header">
        <div>
          ...
          <p className="subtitle">Daily intake and appointment workflow for fictional demo data</p>
        </div>
      ...
      <div className="grid-3">
        ...
        <div className="card" style={{ overflow: "hidden" }}>
          ...
          <div className="utilization">
            {/* Header row */}
            <div className="util-row util-head">
              <div className="name">Doctor / Day</div>
              {next7Days.map((d) => (
                <div key={d} className="util-cell util-date">
                  {d.slice(5)}
                </div>
              ))}
            </div>
      ...
      {/* Timeframe & quick add */}
      <div className="card workflow-toolbar">
        <div className="workflow-toolbar-inner">
