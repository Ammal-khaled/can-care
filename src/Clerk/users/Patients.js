...
const Patients = () => {
  ...
  const isMissingAssignment = (p) =>
    !p.doctorId && !p.doctor && !p.assignedDoctorId && !p.nurseId && !p.nurse && !p.assignedNurseId;

  const filteredPatients = useMemo(() => {
    const s = search.trim().toLowerCase();
    return patients.filter((p) => {
      const matchesSearch = !s || p.name?.toLowerCase().includes(s) || p.id?.toLowerCase().includes(s);
      const matchesFilter = activeFilter === 'missing-assignment' ? isMissingAssignment(p) : true;
  ...
  return (
    <div className="users-page">
      ...
      <div className="users-actions">
        ...
        />
        {(activeFilter || searchParams.get('search')) && (
          <div className="workflow-filter-chip">
            <span>
              Filtered by: {activeFilter === 'missing-assignment' ? 'Missing assignment' : `Search "${searchParams.get('search')}"`}
            </span>
            ...
            {loading ? (
              ...
              <tr>
                <td colSpan="8" className="empty-state">
                  {activeFilter === 'missing-assignment'
                    ? 'No patients are missing care team assignment.'
                    : 'No patients found'}
                </td>
              </tr>
            )}
