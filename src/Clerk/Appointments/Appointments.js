...
import "../../Chief/style/Appointments.css";
...
const Appointments = () => {
  ...
  }, [searchParams, doctors, patients, genId]);
  ...
  const filtered = useMemo(() => {
    ...
    return appointments.filter((a) => {
      ...
      if (selectedDate) {
        return match && a.date === selectedDate && (!searchParams.get("status") || a.status.toLowerCase() === searchParams.get("status").toLowerCase());
      }
      return match && (!searchParams.get("status") || a.status.toLowerCase() === searchParams.get("status").toLowerCase());
    });
  }, [appointments, search, selectedDate, patientById, doctorById, searchParams]);
  ...
  const handleSave = (form) => {
    try {
      ...
      setModal((m) => ({
        ...m,
        err: e.message || "Failed to save appointment",
      }));
    }
  };
  ...
  return (
    <div className="appointments-page">
      ...
      {/* Calendar + Slots */}
      <div className={`appointments-calendar-section ${searchParams.get("action") === "book" ? "workflow-highlight" : ""}`}>
        <Calendar
...
const DeleteModal = ({
  ...
  <div className="modal-overlay">
    <div className="modal">
      ...
      <p>
        Delete appointment for <strong>{patientName}</strong> with {doctorName}{" "}
        on {appointment.date}?
      </p>
      <div className="modal-actions">
